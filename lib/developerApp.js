...
(function (){
  ...
  DeveloperApp.prototype.getAttribute = promiseWrap(function(options, cb) {
    if ( ! cb) { cb = options; options = {}; }
    var conn = this.conn;
    ...
    common.insureFreshToken(conn, function(requestOptions) { 
      requestOptions.url = urljoin(conn.urlBase, 'developers', discriminator, 'apps', name, 'attributes', attribute)
  ...
  DeveloperApp.prototype.updateAttribute = promiseWrap(function(options, cb) {
    if ( ! cb) { cb = options; options = {}; }
    var conn = this.conn;
    ...
    var name = options.appName || options.name || options.app;
    var attribute = options.attribute;
    var value = options.value || options.attributeValue;
    ...
    common.insureFreshToken(conn, function(requestOptions) { 
      requestOptions.headers['content-type'] = 'application/json';
      requestOptions.url = urljoin(conn.urlBase, 'developers', discriminator, 'apps', name, 'attributes', attribute)
      requestOptions.body = JSON.stringify({ "value" :  value });
      if (conn.verbosity>0) {
        utility.logWrite(sprintf('POST %s', requestOptions.url));
      }
