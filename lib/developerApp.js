...
DeveloperApp.prototype.create = promiseWrap(function(options, cb) {
  ...
  const conn = this.conn,
        ...
        email = options.developer || options.developerEmail || options.email,
        product = options.apiProduct || options.productName || options.products || options.apiProducts;
  if ( !email ) {
...
DeveloperApp.prototype.del = promiseWrap(function(options, cb) {
  ...
  let conn = this.conn,
      name = options.appName || options.name || options.app,
      discriminator = options.developer || options.developerId || options.developerEmail || options.email ;
...
function revokeOrApprove(conn, options, cb) {
  ...
  let discriminator = options.developer || options.developerId || options.developerEmail || options.email ;
  if( ! discriminator ) {
  ...
  let urlTail = sprintf('developers/%s/apps/%s', discriminator, options.app || options.appName);
...
DeveloperApp.prototype.update = promiseWrap(function(options, cb) {
  ...
  if ( ! options.scopes && !options.attributes && !options.callbackUrl) {
    return cb(new Error('nothing to update. specify one or more of {attributes, scopes, callbackUrl}'));
  }
  ...
  common.insureFreshToken(conn, function(requestOptions) {
    ...
    request.get(requestOptions, common.callback(conn, [200], function(e, app){
      let attributes1 = common.maybeReformAttributes(options.attributes || {});
