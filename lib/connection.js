...
function rs256Jwt(header, payload, key) {
  ...
  if (header.alg == 'RS256') {
    let signer = crypto.createSign('sha256'),
        signatureBase = [header, payload].map( x => b64(JSON.stringify(x)) ).join('.');
...
Connection.prototype.getExistingToken = promiseWrap(function(cb) {
  let conn = this,
      existingToken = tokenMgmt.currentToken(conn.user, conn.loginBaseUrl, conn.mgmtServer);
...
Connection.prototype.getNewToken = promiseWrap(function(arg1, cb) {
  ...
  if ( typeof arg1 == 'string' ) {
    ...
    formparams = {...formparams, response_type: 'token', passcode: arg1.passcode };
  }
  else if (arg1.password) {
    // this is not well tested
    formparams = {...formparams, username: conn.user || arg1.username, password: arg1.password };
    if (arg1.mfa_token) {
