...
TargetServer.prototype.get = promiseWrap(function(options, cb) {
  // GET :mgmtserver/v1/organizations/:org/environments/:env/targetservers
  // or
  // GET :mgmtserver/v1/organizations/:org/environments/:env/targetservers/:targetserver
  ...
  common.insureFreshToken(conn, function(requestOptions) {
    requestOptions.url = (options.name) ?
      urljoin(conn.urlBase, 'environments', env, 'targetservers', options.name) :
      urljoin(conn.urlBase, 'environments', env, 'targetservers') ;
    if (conn.verbosity>0) {
...
function enableOrDisable(conn, wantEnable, params, cb){
  ...
  common.insureFreshToken(conn, function(requestOptions) {
    requestOptions.url = urljoin(conn.urlBase, 'environments', params.environment, 'targetservers', name);
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('GET %s', requestOptions.url));
    }
...
TargetServer.prototype.update = promiseWrap(function(options, cb) {
  ...
  const payload = options.targetserver || options.target;
  if ( ! payload) {
  ...
  const conn = this.conn;
  common.insureFreshToken(conn, function(requestOptions) {
    requestOptions.url = urljoin(conn.urlBase, 'environments', env, 'targetservers', payload.name);
...
TargetServer.prototype.del = promiseWrap(function(options, cb) {
  ...
  const conn = this.conn,
        env = options.environment || options.env;
  if ( ! env) {
    return cb(new Error('missing required parameter: environment'));
  }
  if ( ! options.name) {
  ...
  common.insureFreshToken(conn, function(requestOptions) {
    requestOptions.url = urljoin(conn.urlBase, 'environments', env, 'targetservers', options.name);
    if (conn.verbosity>0) {
...
TargetServer.prototype.create = promiseWrap(function(options, cb) {
  ...
  const conn = this.conn;
  common.insureFreshToken(conn, function(requestOptions) {
    requestOptions.url = urljoin(conn.urlBase, 'environments', env, 'targetservers');
    requestOptions.headers['content-type'] = 'application/json';
    requestOptions.body = JSON.stringify(payload);
    if (conn.verbosity>0) {
