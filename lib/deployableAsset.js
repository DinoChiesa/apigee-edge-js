...
(function (){
  ...
  function undeploy(conn, options, assetType, cb){
    ...
    var rev = (options.revision)? (options.revision.name || options.revision): null;
    let undeployRevision = function(name, rev){
          if (conn.verbosity>0) {
    ...
    if (rev) {
      ...
      getDeployments(conn, assetType, collection, options, function(e, result) {
        ...
        if (result.environment && result.environment[0] &&
          ...
          let rev = result.environment[0].revision[0].name;
          undeployRevision(options.name, rev);
        }
        else {
