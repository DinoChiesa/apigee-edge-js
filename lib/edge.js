...
// last saved: <2017-April-25 18:38:18>
...
(function (){
  var path = require('path'),
      ...
      gVerbosity = 1,
      gRequestOptions, gUser, gTokenExpiry, gUrlBase, gOrgProperties = null;

//require('request-debug')(request);


  // to handle expiry of the oauth token
  function mergeRequestOptions(cb) {
    if (gRequestOptions.headers && gRequestOptions.headers.Authorization &&
        gUser && gTokenExpiry && gRequestOptions.headers.Authorization.indexOf('Bearer ') === 0) {
      var fudge = 30 * 1000; // in milliseconds
      var now = (new Date()).valueOf() - fudge;
      var stashedToken = tokenMgmt.currentToken(gUser);
      var expiry = tokenMgmt.expiry(stashedToken);
      if (expiry > now) {
        return _refreshToken(expiredToken, function(e, result){
          if (e) {
            throw new Error('error refreshing token: ' + e );
          }
          cb(merge(true, gRequestOptions));
        });
      }
      else {
        cb(merge(true, gRequestOptions));
  ...
  function _invokeTokenEndpoint(formparams, cb) {
    var requestOptions = {
          headers: {
    ...
    request.post(requestOptions, commonCallback([200], function(e, result){
      if (result) {
        gTokenExpiry = (new Date()).valueOf() + result.expires_in * 1000;
        if (gRequestOptions.auth && gRequestOptions.auth.user) {
          gUser = gRequestOptions.auth.user;
        }
  ...
  function _refreshToken(expiredToken, cb) {
    var formparams = {
          refresh_token: expiredToken.refresh_token,
          grant_type : 'refresh_token'
        };
    return _invokeTokenEndpoint(formparams, cb);
  }

  function _getNewToken(cb) {
    var formparams = {
  ...
  function _connect(options, cb) {
    if (options.mgmtServer == gEdgeSaasMgmtServer) {
      // use oauth tokens for Admin API, only for SaaS
      ...
      if (stashedToken && tokenMgmt.isNotExpired(stashedToken)) {
        gRequestOptions.headers.Authorization = 'Bearer ' + stashedToken.access_token;
        delete gRequestOptions.auth;
        return cb(null, stashedToken);
      }
      else if (stashedToken) {
        return _refreshToken(stashedToken, cb);
      }
      else if (!options.no_token) {
        return _getNewToken(cb);
      }
      else {
  ...
  function _getProxy(options, cb) {
    mergeRequestOptions(function(requestOptions) {
