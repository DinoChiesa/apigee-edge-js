...
// last saved: <2017-April-30 17:24:27>
...
//require('request-debug')(request);
  ...
  Organization.prototype.getEnvironments = function(cb) {
    var conn = this.conn;
    if (conn.verbosity>0) {
  ...
  Organization.prototype.refreshToken = function(expiredToken, cb) {
    var conn = this.conn;
    var formparams = {
  ...
  Organization.prototype.getNewToken = function(pass, cb) {
    var conn = this.conn;
    var formparams = {
          username: this.user,
          password: pass,
  ...
  function _connect(options, cb) {
    ...
    if (options.mgmtServer == gEdgeSaasMgmtServer) {
      var stashedToken = tokenMgmt.currentToken(options.user);
      if (stashedToken) {
        org = new Organization(c);
        if (tokenMgmt.isNotExpired(stashedToken)) {
          c.requestHeaders.authorization = 'Bearer ' + stashedToken.access_token;
          return cb(null, org);
        }
        return org.refreshToken(stashedToken, function(e, result){ cb(e, org); });
      }
      else {
        if (!options.password ) {
        ...
        if (!options.no_token) {
          org = new Organization(c);
          return org.getNewToken(options.password, function(e, result){ cb(e, org); });
        }
        else {
  ...
  function mergeRequestOptions(conn, cb) {
    ...
    if (rh && rh.authorization &&
        conn.user && conn.tokenExpiry && rh.authorization.indexOf('Bearer ') === 0) {
      var fudge = 30 * 1000; // in milliseconds
      var now = (new Date()).valueOf() - fudge;
      var stashedToken = tokenMgmt.currentToken(conn.user);
      var expiry = tokenMgmt.expiry(stashedToken);
      if (expiry > now) {
  ...
  function invokeTokenEndpoint(conn, formparams, cb) {
    ...
    request.post(requestOptions, commonCallback(conn, [200], function(e, result) {
      if ( ! e && result) {
        conn.tokenExpiry = (new Date()).valueOf() + result.expires_in * 1000;
  ...
  function internalImportBundleFromZip(conn, assetName, assetType, zipArchive, cb) {
    ...
    mergeRequestOptions(conn, function(requestOptions) {
      ...
      fs.createReadStream(zipArchive)
        .pipe(request.post(requestOptions, commonCallback(conn, [201], cb)));
    });
  }
  ...
  function internalGetEnvironments(conn, cb) {
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'e');
      if (conn.verbosity>0) {
  ...
  Developer.prototype.create = function(options, cb) {
    ...
    if (conn.verbosity>0) {
      common.logWrite(sprintf('Create Developer %s', options.developerEmail || 'unknown'));
    }
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.headers['content-type'] = 'application/json';
  ...
  SharedFlow.prototype.importFromDir = function(name, srcDir, cb) {
    var conn = this.conn;
    ...
    if (conn.verbosity>0) {
      common.logWrite(sprintf('import sharedflow %s from dir %s', name, srcDir));
    }
    return importAssetFromDir(conn, name, srcDir, 'sharedflowbundle', cb);
  };

  SharedFlow.prototype.importFromZip = function(name, zipArchive, cb) {
    ...
    return internalImportBundleFromZip(conn, name, 'sharedflow', zipArchive, cb);
  };
  ...
  ApiProxy.prototype.get = function(options, cb) {
    ...
    requestOptions.url = (options.proxy) ?
        urljoin(conn.urlBase, 'apis', options.proxy) :
        urljoin(conn.urlBase, 'apis');
    if (conn.verbosity>0) {
      common.logWrite(sprintf('GET %s', requestOptions.url));
    }
    request.get(requestOptions, commonCallback(conn, [200], cb));
  ...
  ApiProxy.prototype.del = function(options, cb) {
    ...
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'apis', options.name);
      if (conn.verbosity>0) {
        common.logWrite(sprintf('DELETE %s', requestOptions.url));
      }
      request.del(requestOptions, commonCallback(conn, [200], cb));
  ...
  ApiProxy.prototype.importFromDir = function(proxyName, srcDir, cb) {
    ...
    if (conn.verbosity>0) {
      common.logWrite(sprintf('import proxy %s from dir %s', proxyName, srcDir));
    }
    return importAssetFromDir(conn, proxyName, srcDir, 'apiproxy', cb);
  };

  ApiProxy.prototype.importFromZip = function(proxyName, zipArchive, cb) {
    ...
    return internalImportBundleFromZip(conn, proxyName, 'apiproxy', zipArchive, cb);
  };
