...
// last saved: <2017-August-09 10:04:29>
...
//require('request-debug')(request);
  ...
  function export0(conn, assetType, collection, options, cb) {
    if (!options.name) {
      return cb({error:sprintf("missing name for %s", assetType)});
    }
    var exportOneProxyRevision = function(requestOptions, revision) {
          if ( ! revision){
          ...
          requestOptions.url = urljoin(conn.urlBase, collection, options.name, 'revisions', revision) + '?format=bundle';
    ...
    return mergeRequestOptions(conn, function(requestOptions) {
      if (!options.revision) {
        conn.org.proxies.getRevisions({name:options.name}, function(e, result) {
          if (e) { return cb(e, result); }
          //console.log('got revisions: ' + JSON.stringify(result));
          var latestRevision = result[result.length - 1];
          exportOneProxyRevision(requestOptions, latestRevision);
        });
      }
      else {
        exportOneProxyRevision(requestOptions, options.revision);
      }
    });
  }

  SharedFlow.prototype.export = function(options, cb) {
    // GET :mgmtserver/v1/o/:orgname/sharedflows/:name/revisions/:rev?format=bundle
    var conn = this.conn;
 ...
 function import0(conn, options, assetType, cb) {
  ...
  ApiProxy.prototype.getRevisions = function(options, cb) {
    ...
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'apis', options.name, 'revisions');
      if (conn.verbosity>0) {
        utility.logWrite(sprintf('GET %s', requestOptions.url));
      }
      request.get(requestOptions, commonCallback(conn, [200], cb));
  ...
  ApiProxy.prototype.getResourcesForRevision = function(options, cb) {
    ...
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'apis', options.name, 'revisions', options.revision, 'resources');
      if (conn.verbosity>0) {
        utility.logWrite(sprintf('GET %s', requestOptions.url));
      }
      request.get(requestOptions, commonCallback(conn, [200], cb));
  ...
  ApiProxy.prototype.getPoliciesForRevision = function(options, cb) {
    ...
    mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'apis', options.name, 'revisions', options.revision, 'policies');
      if (options.policy) {
