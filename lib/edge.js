...
(function (){
  ...
  Edge.prototype.connect = promiseWrap(function(options, cb) {
    ...
    const config = (options.keyfile) ? require(options.keyfile): null;
    ...
    var mgmtServer = (config) ? gGoogleMgmtServer :
      trimSlash(options.mgmtServer || gEdgeSaasMgmtServer);
      ...
      if (stashedToken) {
        ...
        if ( tokenMgmt.isInvalidOrExpired(stashedToken)) {
          ...
          return c.refreshToken(stashedToken, function(e, result){
            ...
            // if (c.verbosity) {
            //   utility.logWrite('refresh failed: ' + e.message);
            // }
