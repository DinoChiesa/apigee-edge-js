...
// last saved: <2017-August-08 17:12:08>
...
//require('request-debug')(request);
  ...
  function export0(conn, assetType, collection, options, cb) {
    ...
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('Export %s %s %s', assetType, options.name, options.revision));
    }
    return mergeRequestOptions(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, collection, options.name, 'revisions', options.revision) + '?format=bundle';
      ...
      request.get(requestOptions, commonCallback(conn, [200], function(e, result) {
        ...
        var filename = sprintf('%s-%s-%s-r%s-%s.zip', assetType, conn.org, options.name, options.revision, datestring);
