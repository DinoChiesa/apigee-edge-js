...
// last saved: <2017-May-01 09:51:16>
...
//require('request-debug')(request);
  ...
  function _connect(options, cb) {
    ...
    // or
    // options = {netrc: true, org: "orgname"}
    // or
    // options = {netrc: true, mgmtServer: "https://api.ent.api.com", org: "orgname"}
  ...
  SharedFlow.prototype.importFromDir = function(name, srcDir, cb) {
    ...
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('import sharedflow %s from dir %s', name, srcDir));
    }
  ...
  SharedFlow.prototype.importFromZip = function(name, zipArchive, cb) {
    ...
    var conn = this.conn;
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('import sharedflow %s from zip %s', name, zipArchive));
    }
  ...
  ApiProxy.prototype.del = function(options, cb) {
    ...
    var conn = this.conn;
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('Delete API %s', options.name));
    }
    mergeRequestOptions(conn, function(requestOptions) {
  ...
  ApiProxy.prototype.importFromZip = function(proxyName, zipArchive, cb) {
    ...
    var conn = this.conn;
    if (conn.verbosity>0) {
      utility.logWrite(sprintf('import proxy %s from zip %s', proxyName, zipArchive));
    }
