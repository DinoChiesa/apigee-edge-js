...
(function (){
    ...
    function CompanyApp(conn) { this.conn = conn; }
    ...
    CompanyApp.prototype.create = promiseWrap(function(options, cb) {
        ...
        if( ! disoptions.company ) {
            return cb(new Error("missing company ressource"));
        }
    
    
        let urlTail = sprintf('companies/%s/apps', options.company);
    
        if (conn.verbosity>0) {
            utility.logWrite(sprintf('create %s', urlTail));
        }
      ...
      common.insureFreshToken(conn, function(requestOptions) {
        requestOptions.url = (name) ?
          urljoin(conn.urlBase, 'companies', options.company):
          urljoin(conn.urlBase, 'companies', options.companycriminator, 'apps', name);
