...
(function (){
  ...
  function Team(conn) { this.conn = conn; }
  ...
  Team.prototype.get = promiseWrap(function(options, cb) {
    ...
    common.insureFreshToken(conn, function(requestOptions) {
      ...
      if (team) { requestOptions.url = urljoin(requestOptions.url, team); }
  ...
  Team.prototype.listAudiences = promiseWrap(function(options, cb) {
    ...
    let conn = this.conn,
        zone = options.zone;
    common.insureFreshToken(conn, function(requestOptions) {
      requestOptions.url = urljoin(urlBase(conn), 'providers', zone, 'audiences') + '?organization=' + conn.orgname;
      if (conn.verbosity>0) {
        utility.logWrite(sprintf('GET %s', requestOptions.url));
      }
      request.get(requestOptions, common.callback(conn, [200], cb));
