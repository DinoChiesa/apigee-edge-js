...
function putKvm0(conn, options, cb) {
  common.insureFreshToken(conn, function(requestOptions) {
    ...
    if (conn.orgProperties['features.isCpsEnabled'] || conn.orgInfo.projectId /* hybrid or X */) {
      ...
      request.get(requestOptions, function(error, response, body) {
        ...
        if (conn.orgInfo.projectId) {
          if (response.statusCode == 200) {
            // Delete and re-create is required if the key already exists.
            if (conn.verbosity>0) {
            ...
            request.del(requestOptions, common.callback(conn, [200], function(error, response, body) {
...
Kvm.prototype.get = promiseWrap(function(options, cb) {
  // list KVMs or get info on a single KVM
  let conn = this.conn;
  if ( ! cb) { cb = options; options = {}; }
  let name = options.name || options.kvmName || options.kvm;
  common.insureFreshToken(conn, function(requestOptions) {
    let baseKvmPath = resolveKvmPath(conn, options);
    let getit = function(conn, options, cb) {
          if (name) {
            requestOptions.url = (conn.orgInfo.projectId) ? urljoin(baseKvmPath, name, 'entries') :
              urljoin(baseKvmPath, name);
          }
          else {
            // list kvms
            requestOptions.url = baseKvmPath;
          }
          if (conn.verbosity>0) {
            utility.logWrite(sprintf('GET %s', requestOptions.url));
          }
...
Kvm.prototype.put = promiseWrap(function(options, cb) {
  let conn = this.conn;
  // no need to call common.insureFreshToken(conn, function(requestOptions)
  // conn.org.getProperties will get a fresh token.
  // if we already have org properties, it means the token is fresh.
  if ( ! conn.orgProperties) {
    conn.org.getProperties(function(e, result) {
      if (e) { return cb(e, result); }
