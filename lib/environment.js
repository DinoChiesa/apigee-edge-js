...
(function (){
  ...
  const inquireVhost = function(conn, vhost, options, cb) {
          ...
          common.insureFreshToken(conn, function(requestOptions) {
            requestOptions.url = (vhost) ?
              urljoin(conn.urlBase, 'e', env, 'virtualhosts', vhost):
              urljoin(conn.urlBase, 'e', env, 'virtualhosts');
            if (conn.verbosity>0) {
              utility.logWrite(sprintf('GET %s', requestOptions.url));
            }
  ...
  Environment.prototype.getVhosts = promiseWrap(function(options, cb) {
    let conn = this.conn;
    let vhost = options.vhost || options.vhostName || options.virtualhost || options.name;
    if (vhost) {
  ...
  Environment.prototype.createVhost = promiseWrap(function(options, cb) {
    ...
    common.insureFreshToken(conn, function(requestOptions) {
      requestOptions.url = urljoin(conn.urlBase, 'e', env, 'virtualhosts');
      if (conn.verbosity>0) {
