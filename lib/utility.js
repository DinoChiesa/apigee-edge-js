...
(function (){
  ...
  function verifyCommonRequiredParameters(options, getopt) {
    ...
    if (options.netrc) {
      ...
      options.user = netrc[parsedAuthUrl.hostname].login;
      options.password = netrc[parsedAuthUrl.hostname].password;
    }

    if ( !options.user) {
      options.user = readlineSync.question(' USER NAME  : ');
    }

    if ( ! options.notoken) {
      let stashedToken = tokenMgmt.currentToken(options.user, loginBaseUrl(options), options.mgmtserver);
      if ( stashedToken ) {
        options.token = stashedToken;
      }
    }

    if ( ! options.token && !options.passcode && !options.password) {
      options.password = readlineSync.question(' Password for ' + options.username + ' : ',
                                               {hideEchoBack: true});
    }

    if ( !options.token && !options.passcode && (!options.user || !options.password)) {
