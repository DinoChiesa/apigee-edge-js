...
Organization.prototype.removeProperties = promiseWrap(
  function (propertyArray, cb) {
    const conn = this.conn;
    let propertyHash = {};
...
Organization.prototype.setConsumerSecretLength = promiseWrap(
  function (length, cb) {
    const conn = this.conn;
    ...
    postProps(
      conn,
      { "keymanagement.consumer.secret.length": length },
      "overwrite",
      cb,
    );
  },
...
Organization.prototype.setConsumerKeyLength = promiseWrap(
  function (length, cb) {
    const conn = this.conn;
    if (typeof length != "number" || length < 16 || length > 128) {
      return cb(new Error("invalid argument"));
    ...
    postProps(
      conn,
      { "keymanagement.consumer.key.length": length },
      "overwrite",
      cb,
    );
  },
...
Organization.prototype.getName = function () {
  const conn = this.conn;
  return Promise.resolve(conn.orgname);
};
